...
function setReference(ref){
 ...
 state.ref=ref;state.reads=[];state.rows=[];$('readselect').replaceChildren();$('alignments').replaceChildren();$('variants').replaceChildren();$('v7rows').replaceChildren();$('v7status').textContent='Référence modifiée : relancez l’analyse ABI.';$('cdsstart').value=ref.cdsStart||'';
...
$('analyze').onclick=async()=>{state.reads=[];state.rows=[];$('v7status').textContent='Analyse ABI en cours…';$('variants').replaceChildren();$('alignments').replaceChildren();$('readselect').replaceChildren();try{
 ...
 $('v7status').textContent='ABI chargés : vous pouvez lancer l’analyse indépendante.'; $('status').textContent=`${state.reads.length} chromatogramme(s) lu(s) ; ${state.rows.length} discordance(s) simples indicatives. ${state.ref?'Référence présente.':'Aucune référence : visualisation uniquement.'}`;draw();
 }catch(e){$('status').textContent='Erreur : '+e.message;$('v7status').textContent='Analyse indisponible : '+e.message;console.error('SVIB analyse ABI',e)}}
